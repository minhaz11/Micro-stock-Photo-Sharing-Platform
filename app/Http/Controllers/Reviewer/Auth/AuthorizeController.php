    ...
    public function authorizeForm()
    {
        ...
        if (auth()->guard('reviewer')->check()) {
            ...
            if (!$user->status) {
                ...
                return view('reviewer.auth.authorization.email', compact('user', 'page_title'));
            }elseif (!$user->sv) {
                if (!$this->checkValidCode($user, $user->ver_code)) {
                    $user->ver_code = verificationCode(6);
                    $user->ver_code_send_at = Carbon::now();
                ...
                return view('reviewer.auth.authorization.sms', compact('user', 'page_title'));
            }elseif (!$user->tv) {
                $page_title = 'Google Authenticator';
                return view('reviewer.auth.authorization.2fa', compact('user', 'page_title'));
            }else{
