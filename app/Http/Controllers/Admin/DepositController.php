...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        if($deposit->plan_id != null){
            $user->balance -=  $deposit->amount;
            $user->save();

            $plan = Subscription::findOrFail($deposit->plan_id);
            ...
            if($user->ref_by!=null){
                $refUser = User::findOrFail($user->ref_by);
                $bonus = $deposit->amount*($gnl->ref_bonus/100);
        ...
        if($deposit->plan_id != null){
            notify($deposit->user, 'SUBSCRIPTION', [
