...
class TicketController extends Controller
{
                ...
                function ($attribute, $value, $fail) use ($files, $allowedExts) {
                    foreach ($files as $file) {
                        ...
                        if (($file->getSize() / 1000000) > 2) {
                            return $fail("Files MAX  2MB ALLOW!");
                        }
    ...
    public function viewTicket($ticket)
    {
        ...
        if($user){
            ...
            return view($this->activeTemplate. 'user.support.guestView', compact('my_ticket', 'messages', 'page_title'));
        }
                    ...
                    function ($attribute, $value, $fail) use ($imgs, $allowedExts) {
                        foreach ($imgs as $img) {
                            ...
                            if (($img->getSize() / 1000000) > 2) {
                                return $fail("Files MAX  2MB ALLOWED!");
                            }
                            if (!in_array($ext, $allowedExts)) {
                                return $fail("Only png, jpg, jpeg, pdf doc docx files are allowed");
                            }
